...
export function BuyerDemandMonitor() {
  ...
  async function runMonitor() {
    ...
    try {
      ...
      if (inserted > 0) {
        // Keep the coverage report visible and update the server-rendered queue
        // in place instead of navigating away from the completed run.
        router.refresh();
      }
      setRunning(false);
    } catch (error) {
  ...
  return <section className="buyerDemandMonitor"><div><span className="eyebrow">ACTIVE BUYER DEMAND</span>
    ...
    <div className="monitorAction"><button type="button" onClick={runMonitor} disabled={running}>{running ? "正在搜索并核验来源……" : "运行一轮采购需求增量搜索"}</button>{message && <small>{message}</small>}<small className="parallelHint">多组来源并行搜索；耗时短不代表只执行了一次，请以“已执行 X/X 组”和检查网页数为准。</small></div>
    <style>{`.buyerDemandMonitor{margin:28px 0 0;display:flex;justify-content:space-between;gap:28px;align-items:center;padding:24px;border:1px solid #bcd8d0;border-radius:18px;background:linear-gradient(135deg,#f4fbf8,#fffdf8)}.buyerDemandMonitor h2{font:30px Georgia,serif;margin:5px 0 8px}.buyerDemandMonitor p{max-width:840px;margin:0;color:#5f6e68;line-height:1.65}.monitorMode{display:grid;gap:8px}.monitorMode label{display:flex;gap:8px;align-items:center}.monitorMode select{border:1px solid #b5c9c2;border-radius:8px;padding:6px 8px;font-size:12px}.modeHint{margin-top:8px;font-size:12px;color:#6a4e00;background:#fff6db;padding:4px 9px;border-radius:10px;display:inline-block;max-width:380px}.demandRules{display:flex;flex-wrap:wrap;gap:7px;margin-top:13px}.demandRules span{padding:5px 9px;border-radius:999px;background:#e1f1eb;color:#245947;font-size:12px;font-weight:700}.monitorAction{min-width:260px;display:grid;gap:9px}.monitorAction button{border:0;border-radius:10px;padding:13px 16px;background:#176b55;color:#fff;font-weight:800;cursor:pointer}.monitorAction button:disabled{opacity:.55;cursor:wait}.monitorAction small{max-width:310px;color:#5f6e68;line-height:1.5}.monitorAction .parallelHint{color:#7c5a00;background:#fff7d6;border-radius:8px;padding:6px 8px}@media(max-width:800px){.buyerDemandMonitor{display:block}.monitorAction{margin-top:18px;min-width:0}}`}</style>
  </section>;
}
